...
declare type PartialK<T, K extends PropertyKey = PropertyKey> = Partial<Pick<T, Extract<keyof T, K>>> & Omit<T, K> extends infer O
	? { [P in keyof O]: O[P] }
	: never;

declare type RequiredK<T, K extends PropertyKey = PropertyKey> = Required<Pick<T, Extract<keyof T, K>>> & Omit<T, K> extends infer O
	? { [P in keyof O]: O[P] }
	: never;
