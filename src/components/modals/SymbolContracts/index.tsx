...
import { type IContractSelectorModal } from '@/features/slices/modalSlice.interfaces';
...
const SymbolContracts = forwardRef<HTMLDivElement, SymbolContractsProps>(
	({ symbolISIN, symbolTitle, ...props }, ref) => {
		...
		return (
			<Modal top='7.2rem' onClose={onCloseModal} {...props} ref={ref}>
								...
								<button
									...
									className={cn(
										...
										JSON.stringify(item) === JSON.stringify(states.activeSettlement)
											? 'no-hover justify-start font-medium btn-select'
											: 'border-gray-500 bg-white text-gray-1000 hover:btn-hover',
									)}
								>
									{t('symbol_contracts_modal.end_date') +
										' ' +
										dayFormatter(item.contractEndDate ?? '')}
								</button>
							))}
						</div>
