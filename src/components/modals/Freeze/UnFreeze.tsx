		...
		<div className='flex h-full flex-col justify-between'>
			<div className='mb-16 mt-24 flex-1 p-8 shadow-lg'>
				<div className='mb-24 flex'>
					<span className='text-gray-700 flex-1 text-center'>{t('freeze_modal.symbol_title')}</span>
					<span className='text-gray-700 flex-1 text-center'>{t('freeze_modal.guaranted_number')}</span>
				</div>
				...
				<div className='flex flex-col gap-y-16 overflow-auto' style={{ maxHeight: 300 }}>
					{data?.map((item, ind) => (
						<div className='flex' key={ind}>
							...
							<span className='text-gray-800 flex-1 text-center'>{sepNumbers(String(item.count))}</span>
						</div>
					))}
				</div>
			</div>
