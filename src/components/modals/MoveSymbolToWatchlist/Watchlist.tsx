...
const Watchlist = ({ top, isActive, watchlist, onSelect }: WatchlistProps) => (
	<li
		style={{ top: `${top}rem` }}
		onClick={onSelect}
		className='absolute h-64 w-full cursor-pointer rounded px-8 transition-bg flex-justify-between hover:bg-secondary-100'
	>
		<h4 className='text-gray-800'>{watchlist.name}</h4>

		<button type='button' className={isActive ? 'text-warning-100' : 'text-gray-700'}>
