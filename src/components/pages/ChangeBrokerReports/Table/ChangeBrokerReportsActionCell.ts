...
type ChangeBrokerReportsActionCellProps = ICellRendererParams<Reports.IChangeBrokerReports, string> & {
	onDeleteRow?: (data: Reports.IChangeBrokerReports | undefined) => Promise<void>;
};

class ChangeBrokerReportsActionCell implements ICellRendererComp<Reports.IChangeBrokerReports> {

	params!: ChangeBrokerReportsActionCellProps;
	...
	init(params: ChangeBrokerReportsActionCellProps) {
	...
	refresh(params: ChangeBrokerReportsActionCellProps) {
		this.params = params;
		return true;
	}

	private createDeleteBtn(params: ChangeBrokerReportsActionCellProps): HTMLButtonElement {
		const btn = document.createElement('button');
		btn.setAttribute('class', 'flex-justify-center text-gray-900 rounded px-4');
		btn.disabled = String(params.data?.lastState) !== 'Draft';
...
export default ChangeBrokerReportsActionCell;
