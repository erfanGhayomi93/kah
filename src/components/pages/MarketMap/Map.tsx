	...
	const onAfterDraw = () => {
		...
		const stockInfo = eGroup
			...
			.on('mouseover', (_, node) => {
				...
				const title = `<div class="border-b border-b-gray-200 text-right text-base pb-8"><span>${data.st}</span></div>`;
				const lastTradedPrice = `<div class="text-sm flex justify-between gap-16"><div class="flex gap-8"><span>${sepNumbers(String(data.l | 0))}</span><span class="text-left ${lastTradedPricePercent < 0 ? 'text-error-100' : 'text-success-200'}">${lastTradedPricePercent < 0 ? `(${Math.abs(lastTradedPricePercent).toFixed(2)}%)` : `${lastTradedPricePercent.toFixed(2)}%`}</span></div><span class="text-right">آخرین قیمت</span></div>`;
				const closingPrice = `<div class="text-sm flex justify-between gap-16"><div class="flex gap-8"><span>${sepNumbers(String(data.c | 0))}</span><span class="text-left ${closingPricePercent < 0 ? 'text-error-100' : 'text-success-200'}">${closingPricePercent < 0 ? `(${Math.abs(closingPricePercent).toFixed(2)}%)` : `${closingPricePercent.toFixed(2)}%`}</span></div><span class="text-right">قیمت پایانی</span></div>`;
