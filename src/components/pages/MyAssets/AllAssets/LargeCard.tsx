	...
	<div {...props} style={{ height: '42.8rem', ...style }}>
		<div className='h-full gap-16 rounded pb-16 pl-8 pr-16 pt-8 shadow-card flex-column'>
			<div className='h-40 flex-justify-between'>
				<h2 className='text-lg font-medium text-light-gray-700'>{title}</h2>
				...
				{Array.isArray(tabs) && tabs.length > 0 && (
					<SwitchTab<TabProps<T>>
						...
						classes={{
							root: '!h-40 bg-light-gray-100 !border-0 py-4 px-8',
						...
						renderTab={(item, activeTab) => (
							<button
								...
								className={clsx(
									...
									item.id === activeTab ? 'font-medium text-light-gray-800' : 'text-light-gray-500',
								)}
							>
								{item.title}
							</button>
