...
class StrategyActionCell implements ICellRendererComp<unknown> {
	...
	init(params: StrategyActionCellProps) {
		...
		if (this.params.execute) this.eGui.appendChild(this.createExecuteBtn());
	}
	...
	private createExecuteBtn(): HTMLButtonElement {
		...
		try {
			...
			btn.setAttribute('class', 'flex-justify-center disabled:opacity-50 text-gray-700 rounded px-4');
