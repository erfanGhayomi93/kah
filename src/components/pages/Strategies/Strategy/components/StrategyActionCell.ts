...
type StrategyActionCellProps = ICellRendererParams<unknown, number> & {
	goToTechnicalChart?: (data: unknown) => void;
...
class StrategyActionCell implements ICellRendererComp<unknown> {
	...
	init(params: StrategyActionCellProps) {
		...
		this.eGui.appendChild(this.createTechnicalBtn());
	...
	private createTechnicalBtn(): HTMLButtonElement {
		const btn = document.createElement('button');
		btn.setAttribute('class', 'flex-justify-center text-gray-900 rounded px-4');
		btn.onclick = () => this.params.goToTechnicalChart?.(this.params.data!);
		btn.innerHTML =
	...
	private createExecuteBtn(): HTMLButtonElement {
		const btn = document.createElement('button');
		btn.setAttribute('class', 'flex-justify-center text-success-100 rounded px-4');
		btn.onclick = () => this.params.execute?.(this.params.data!);
		btn.innerHTML =
