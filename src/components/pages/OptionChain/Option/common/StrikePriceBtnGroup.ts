...
class StrikePriceBtnGroup {
	...
	private _createTechnicalChartBtn() {
		const btn = this.createBtn();
		btn.type = 'button';
		btn.classList.add('hover:bg-gray-100', 'size-32');
		btn.innerHTML =
	...
	private _createDropdownBtn() {
		...
		this.eDropdownBtn.classList.add('hover:bg-gray-100', 'size-32');
	...
	private createDropdown() {
		try {
			...
			aBtn.setAttribute(
				'class',
				'w-full h-48 text-base px-16 gap-8 text-gray-700 flex-justify-end hover:btn-hover transition-colors',
			);
			aBtn.type = 'button';
			aBtn.innerHTML =
